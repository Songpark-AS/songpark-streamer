...
//			info( "axi_to_net_reg_w_error count is::%d\n",axi_to_net_reg_w_error_count);
    ...
    //------------------------------------------------------------------------------------------------------
                	  ...
                	  int d = get_pkt_seq_skiped(eth_packet_sequencer_base_0);
                      	  ...
                      	  info("Pkt seq get skipped pakets:%d\n>",d);
                      	  oled_clear(zedboard_oled_params_0.base_address);
                      	  ...
                      	  ui_draw();
                      	  	...
                      	  	set_drop_allowance(eth_to_audio_base_0, allw);//wValue(7);
                        		  info("Ethernet to Audio drop allowance set\n");
                        		  oled_clear(zedboard_oled_params_0.base_address);
            		  ...
            		  info("Ethernet to Audio even packet wait allowance set\n");
            		  oled_clear(zedboard_oled_params_0.base_address);
            		  ...
            		  ui_draw();
                  		  ...
                  		  info("Ethernet to Audio odd packet wait allowance set\n");
                  		  oled_clear(zedboard_oled_params_0.base_address);
    ...
    //------------------------------------------------------------------------------------------------------
			...
			ui_draw();
			  ...
			  ui_draw();
				...
				ui_draw();
					...
					ui_draw();
							  ...
							  sig_tpx("{\"tpx_msg\":\"bp__version\", \"version\":\"%s\"}", VER);
							ui_draw();
						  }
		...
		else if(strcmp (c,"cver") == 0)
				{
					...
					oled_print_message(&menuBuf[0], 1, zedboard_oled_params_0.base_address);
					 sig_tpx("{\"tpx_msg\":\"core__version\", \"version\":%0.1f}", tmp/10.0);
					  ui_draw();
				}
			  ...
			  tmp = get_tms_delay_mean(time_sync_base_0);
			  sig_tpx("{\"tpx_msg\":\"stream_param\", \"latency\":\"%0.1f ms\"}", (tmp/SAMPLE_RATE));
			  oled_clear(zedboard_oled_params_0.base_address);
...
//					  ui_draw();
			...
			info("The running media packets latency is: %0.1f ms\n",tmp/SAMPLE_RATE);
			sig_tpx("{\"tpx_msg\":\"stream_param\", \"mlat\":\"%0.1f ms\"}", (tmp/SAMPLE_RATE));
			  oled_clear(zedboard_oled_params_0.base_address);
			  sprintf(&menuBuf[0], "%s", "MDLAY: ..");
			  oled_print_message(&menuBuf[0], 1, zedboard_oled_params_0.base_address);
			  ui_draw();
...
//    	  set_sync_off(audio_to_eth_base_0);
	...
	//				init_incall_timed_task_handler();// In the SIP UA layer
			info("Stream tx started status was:0x%X\n>",buf);
			sig_tpx("{\"tpx_msg\":\"stream_started\"}");
	//		sleep(1);
	//		initiate_sync(time_sync_base_0);
	//		set_sync_on(audio_to_eth_base_0);
	//		info("Enable time sync...\n");
			...
			ui_draw();
    		  ...
    		  sig_tpx("{\"tpx_msg\":\"stream_error\"}");
    		  info("Stream tx not started \n>");
    	  }
...
//    	  sig_tpx("{\"tpx_msg\":\"stream_stopped\"}");
    	  stop_hw_streaming();
		...
		oled_print_message(&menuBuf[0], 2, zedboard_oled_params_0.base_address);
		sig_tpx("{\"tpx_msg\":\"setmac\", \"mac\":\"%s\"}", ip);
		  ui_draw();
	}
			...
			oled_print_message(&menuBuf[0], 2, zedboard_oled_params_0.base_address);
			sig_tpx("{\"tpx_msg\":\"setsip\", \"sip\":\"%s\"}", ip);
      		  ui_draw();
      	}
		...
		sprintf(&menuBuf[0], "%s", ip);
		oled_print_message(&menuBuf[0], 2, zedboard_oled_params_0.base_address);
		sig_tpx("{\"tpx_msg\":\"setdip\", \"dip\":\"%s\"}", ip);
		  ui_draw();
	}
    	  ...
    	  scanf("%s",ip);
			...
			sprintf(&menuBuf[0], "%s", ip);
			oled_print_message(&menuBuf[0], 2, zedboard_oled_params_0.base_address);
			sig_tpx("{\"tpx_msg\":\"setlip\", \"lip\":\"%s\"}", ip);
      		  ui_draw();
      	}
...
//				write_eth_param(full_udp_stack_ip_base_0, LOCAL_IP_ID, (ipAddress[0]<<24)|(ipAddress[1]<<16)|(ipAddress[2]<<8)|(ipAddress[3]));
				...
				sprintf(&menuBuf[0], "%s", ip);
				oled_print_message(&menuBuf[0], 2, zedboard_oled_params_0.base_address);
				sig_tpx("{\"tpx_msg\":\"setrlip\", \"rlip\":\"%s\"}", ip);
				  ui_draw();
			}

      else if(strcmp (c,"setrpip") == 0) {
      				  //SET_STATUS("Enter Public IP Address (xxx.xxx.xxx.xxx format): ");
        	    	  scanf("%s",ip);
//      			  sprintf(ip, "%s",strtok(NULL, token));
      				  extractIpAddress(ip,&ipAddress[0]);
      				  info( "\nIp Address: %03d. %03d. %03d. %03d\n",ipAddress[0],ipAddress[1],ipAddress[2],ipAddress[3]);
      //				write_eth_param(full_udp_stack_ip_base_0, LOCAL_IP_ID, (ipAddress[0]<<24)|(ipAddress[1]<<16)|(ipAddress[2]<<8)|(ipAddress[3]));
      				...
      				sprintf(&menuBuf[0], "%s", ip);
      				oled_print_message(&menuBuf[0], 2, zedboard_oled_params_0.base_address);
      				sig_tpx("{\"tpx_msg\":\"setrpip\", \"rpip\":\"%s\"}", ip);
      				  ui_draw();
      			}
...
//            		write_eth_param(full_udp_stack_ip_base_0, LOCAL_IP_ID, (ipAddress[0]<<24)|(ipAddress[1]<<16)|(ipAddress[2]<<8)|(ipAddress[3]));
      			...
      			sprintf(&menuBuf[0], "%s", ip);
      			oled_print_message(&menuBuf[0], 2, zedboard_oled_params_0.base_address);
      			sig_tpx("{\"tpx_msg\":\"setpip\", \"pip\":\"%s\"}", ip);
            		  ui_draw();
            	}
			...
			sig_tpx("{\"tpx_msg\":\"setsip\", \"sip\":\"%s\"}", ip);
				...
				sprintf(&menuBuf[0], "%s", ip);
				oled_print_message(&menuBuf[0], 2, zedboard_oled_params_0.base_address);
				sig_tpx("{\"tpx_msg\":\"setgip\", \"gip\":\"%s\"}", ip);
           		  ui_draw();
           	}
		  ...
		  sprintf(&menuBuf[0], "%s", menuitem[15]);
			...
			sprintf(&menuBuf[0], "%s", ip);
			oled_print_message(&menuBuf[0], 2, zedboard_oled_params_0.base_address);
			sig_tpx("{\"tpx_msg\":\"setmask\", \"mask\":\"%s\"}", ip);
		  ui_draw();
     }
              ...
              if(check_port(dest_port) == 0){
//            	  sig_tpx("{\"tpx_msg\":\"port_set\"}");
				  ...
				  oled_print_message(&menuBuf[0], 2, zedboard_oled_params_0.base_address);
				  sig_tpx("{\"tpx_msg\":\"setport\", \"port\":%d}", call_params.stream_port);
				  info("Port set:: %d", call_params.stream_port);
				  ui_draw();
              }else{
            	 sig_tpx("{\"tpx_msg\":\"port_in_use\"}");
            	 sig_tpx("{\"tpx_msg\":\"call__params\",\"port\":%d}",call_params.stream_port);
            	 info("Port in use:: %d effective port ", dest_port,call_params.stream_port);
            	 oled_clear(zedboard_oled_params_0.base_address);
				  sprintf(&menuBuf[0], "%s", "Port in use:");
				oled_print_message(&menuBuf[0], 1, zedboard_oled_params_0.base_address);
				sprintf(&menuBuf[0], "%d", dest_port);
				oled_print_message(&menuBuf[0], 2, zedboard_oled_params_0.base_address);
				  ui_draw();
...
//                              }
      ...
      else if(strcmp (c,"setplcpktsize") == 0)
      {
		...
		oled_print_message(&menuBuf[0], 1, zedboard_oled_params_0.base_address);
		sig_tpx("{\"tpx_msg\":\"bp__param\", \"delay\":%d}", tmp);
		ui_draw();
      }
			...
			oled_print_message(&menuBuf[0], 2, zedboard_oled_params_0.base_address);
			ui_draw();
			sig_tpx("{\"tpx_msg\":\"bp__param\", \"delay\":%f}", tmp/SAMPLE_RATE);
  ...
  //		  printf("The current audio rxtcfout is:%0.3f ms\n",(tmp/SAMPLE_RATE));
		  ...
		  info("The num of dropped pkts in dpkt is:%d pks\n",(tmp));
		  tmp = get_pkts_dropped(eth_packet_sequencer_base_0);
		  info("The number of sq packets dropped is:%d\n",tmp);
		  tmp = get_pkt_seq_skiped(eth_packet_sequencer_base_0);
		  info("The number of sq packets skipped is:%d\n",tmp);
		tmp = get_dpkt_replace_events(eth_to_audio_base_0);
    	  info("The number of dpkt replace events is:%d\n",tmp);
    	  //    	  tmp = get_media_latency(time_sync_base_0);
    	  ...
    	  set_buf_corr_on(eth_to_audio_base_0);
			  ...
			  ui_draw();
      		  ...
      		  //extractMacAddress(ip,&macAddress[0]);
      		  //info( "\nMac Address: %02x. %02x. %02x. %02x. %02x. %02x\n",macAddress[0],macAddress[1],macAddress[2],macAddress[3],macAddress[4],macAddress[5]);
      		tmp = read_eth_mac(full_udp_stack_ip_base_0, MAC_REG_ID,&ip[0]);
      		if(tmp > 0){
...
//      		      		   print_mac(tmp, ip);
      		...
      		sig_tpx("{\"tpx_msg\":\"bp__param\", \"mac\":%s}", ip);
          			...
          			sig_tpx("{\"tpx_msg\":\"bp__error\", \"code\":\"nomac\"}");
          			error( "\n Mac  read error...");
          		}
      		  ui_draw();
      	}
            ...
            //info( "\nIp Address: %03d. %03d. %03d. %03d\n",ipAddress[0],ipAddress[1],ipAddress[2],ipAddress[3]);
      		...
      		info( "\nDest Ip Address:");
    			...
    			sig_tpx("{\"tpx_msg\":\"bp__error\", \"code\":\"noip\"}");
    			error( "\n Dest Ip read error...");
    		}
      		  ui_draw();
      	}
            	...
            	if(tmp > 0){
                 		...
                 		sprintf(&menuBuf[0], "%s", menuitem[14]);
                			...
                			sig_tpx("{\"tpx_msg\":\"bp__error\", \"code\":\"nogip\"}");
                			error( "\n GW Ip read error...");
                		}
                 		  ui_draw();
                 	}
      		...
      		info( "\nMask Ip Address:");
      		  ...
      		  sprintf(&menuBuf[0], "%s", menuitem[15]);
					...
					sig_tpx("{\"tpx_msg\":\"bp__error\", \"code\":\"nomask\"}");
					error( "\nMask Ip read error...");
				}
      		  ui_draw();
           }
          		...
          		if(dest_port > 0){
					   info( "The port is:%d\n",dest_port);
						...
						sig_tpx("{\"tpx_msg\":\"call__param\", \"port\":%d}", dest_port);
           					...
           					sig_tpx("{\"tpx_msg\":\"bp__error\", \"code\":\"noport\"}");
           					error( "\nPort read error...");
           				}
                    ui_draw();
                  }
						...
						info( "The sync port is:%d\n",dest_port);
							...
							sprintf(&menuBuf[0], "%d", dest_port);
							oled_print_message(&menuBuf[0], 2, zedboard_oled_params_0.base_address);
							sig_tpx("{\"tpx_msg\":\"call__param\", \"sport\":%d}", dest_port);
						}else{
							sig_tpx("{\"tpx_msg\":\"bp__error\", \"code\":\"nosport\"}");
							error( "\nPort read error...");
						}
							ui_draw();
                  }
...
//				  tmp = get_pkts_dropped(packet_time_enforcer_base_0);
		...
		else if(strcmp (c,"getsqos") == 0)
		{
			...
			if(set_call_params() == 0){
				print_switch_param(hyb_switch_ip_base_0);
				info("Call started waiting \n");
				sig_tpx("{\"tpx_msg\":\"call__state\", \"state\":\"waiting\"}");
				if(init_hw_sync() == 0){
					if(check_sync()	== 0){
						start_hw_streaming();
						sig_tpx("{\"tpx_msg\":\"call__state\", \"state\":\"incall\"}");
						init_incall_timed_task_handler();
					}else{
						//error
	//					info( "\n SYNC WAIT TIMEOUT \n");
	    		  ...
	    		  sig_tpx("{\"tpx_msg\":\"call_error\"}");
	    		  info("Call not started check params\n");
	    	  }
		}
        ...
        info("|==================+================================================|\n");
        info("|vol <gain value>  |volume gain for global level					 |\n");
        info("|------------------+------------------------------------------------|\n");
        info("|vll <gain value>  |volume gain for left channel of loopback stream |\n");
        info("|------------------+------------------------------------------------|\n");
        info("|vlr <gain value>  |volume gain for right channel of loopback stream|\n");
        info("|------------------+------------------------------------------------|\n");
        info("|vnl <gain value>  |volume gain for left channel of network stream  |\n");
        info("|------------------+------------------------------------------------|\n");
        info("|vnr <gain value>  |volume gain for right channel of network stream |\n");
        info("|------------------+------------------------------------------------|\n");
        info("|    lfhe/lfhd     |High pass enable/Disable for loopback           |\n");
        info("|------------------+------------------------------------------------|\n");
        info("|    lfbe/lfbd     |Band pass enable/Disable for loopback           |\n");
        info("|------------------+------------------------------------------------|\n");
        info("|    lfle/lfld     |Low pass enable/Disable for loopback            |\n");
        info("|------------------+------------------------------------------------|\n");
        info("|    nfhe/nfhd     |High pass enable/Disable for network            |\n");
        info("|------------------+------------------------------------------------|\n");
        info("|    nfbe/nfbd     |Band pass enable/Disable for network            |\n");
        info("|------------------+------------------------------------------------|\n");
        info("|    nfle/nfld     |Low pass enable/Disable for network             |\n");
        info("|------------------+------------------------------------------------|\n");
        info("|    nstats        |Byte counts and error counts for the network    |\n");
        info("|------------------+------------------------------------------------|\n");
        info("|    astatus       |Hardware ADC audio FIFO Status                  |\n");
        info("|------------------+------------------------------------------------|\n");
        info("|    dstatus       |Hardware DAC audio FIFO Status                  |\n");
        info("|------------------+------------------------------------------------|\n");
        info("|    areset        |Hardware ADC audio FIFO Reset                   |\n");
        info("|------------------+------------------------------------------------|\n");
        info("|    reset        |Hardware DAC audio FIFO Reset                   |\n");
        info("|------------------+------------------------------------------------|\n");
        info("|    freset        | Resets the udp core and the audio fifos.       |\n");
        info("|------------------+------------------------------------------------|\n");
        info("|    preset   		| Resets the packet sequencer  					 |\n");
        info("|------------------+------------------------------------------------|\n");
        info("|    creset 		| Resets just the udp core.						 |\n");
        info("|------------------+------------------------------------------------|\n");
        info("|    rxrsync 		| Re-syncronise the receive audio chain       	 |\n");
        info("|------------------+------------------------------------------------|\n");
		info("|    strstart      |Start streaming                                 |\n");
        info("|------------------+------------------------------------------------|\n");
        info("|    strstop       |Stop streaming                                  |\n");
        info("|------------------+------------------------------------------------|\n");
        info("|    strseleth     |Play ETH stream                                 |\n");
        info("|------------------+------------------------------------------------|\n");
        info("|    strselarm     |Play ARM stream                                 |\n");
        info("|------------------+------------------------------------------------|\n");
        info("|    strdip <ip>   |Streaming destination IP                        |\n");
        info("|------------------+------------------------------------------------|\n");
        info("|    strlip <ip>   |Streaming local IP                              |\n");
        info("|------------------+------------------------------------------------|\n");
        info("|    strport <port>|Streaming port                                  |\n");
        info("|------------------+------------------------------------------------|\n");
        info("|    strgip <ip>   |Streaming gateway IP                            |\n");
        info("|------------------+------------------------------------------------|\n");
        info("|    strmask <ip>  |Streaming netmask IP                            |\n");
        info("|------------------+------------------------------------------------|\n");
        info("|    strmac <mac>  |Streaming MAC Address                           |\n");
        info("|------------------+------------------------------------------------|\n");
        info("|    strnsp <sp>   |Streaming net speed                             |\n");
        info("|------------------+------------------------------------------------|\n");
        info("|    strplen <len> |Streaming payload packet length                 |\n");
        info("|------------------+------------------------------------------------|\n");
        info("|  strporder <ord> |Set streaming packet sequencer order      |\n");
        info("|------------------+------------------------------------------------|\n");
		info("|    getdip       |Read streaming destination IP                   |\n");
		info("|------------------+------------------------------------------------|\n");
		info("|    getlip       |Read streaming local IP                         |\n");
		info("|------------------+------------------------------------------------|\n");
		info("|    getport      |Read streaming port                             |\n");
		info("|------------------+------------------------------------------------|\n");
		info("|    getgip       |Read streaming gateway IP                       |\n");
		info("|------------------+------------------------------------------------|\n");
		info("|    getmask      |Read streaming netmask IP                       |\n");
		info("|------------------+------------------------------------------------|\n");
		info("|    getmac       |Read streaming MAC Address                      |\n");
		info("|------------------+------------------------------------------------|\n");
		info("|    getnsp       |Read streaming net speed                        |\n");
		info("|------------------+------------------------------------------------|\n");
		info("|    getplen      |Read streaming payload packet length            |\n");
		info("|------------------+------------------------------------------------|\n");
		info("|    getblen      |Read streaming rx sample buffer length          |\n");
		info("|------------------+------------------------------------------------|\n");
	    info("|    getporder    |Read streaming packet sequencer order            |\n");
        info("|------------------+------------------------------------------------|\n");
...
void ui_vol_draw () {
    system ("clear");
    info("|============+============+============|\n");
    info("| Parameters +  Loopback  +   Network  |\n");
    info("|============+============+============|\n");
...
//					  ui_draw();
				...
				info("The media packets is: %0.1f ms\n",tmp/SAMPLE_RATE);
				  oled_clear(zedboard_oled_params_0.base_address);
				  sprintf(&menuBuf[0], "%s", "MDLAY: ..");
				  oled_print_message(&menuBuf[0], 1, zedboard_oled_params_0.base_address);
				  ui_draw();
...
int set_hw_local_net(){
	...
	//  sprintf(mask_ip, argv[5]);
	  ...
	  if (getGateway(gateway_ip) == 0){
		  ...
		  sig_tpx("{\"tpx_msg\":\"bp__error\", \"code\":\"nogw\"}");
		  rc = (-1);
	  }
	  ...
	  if(getLocalIP(local_ip) == 0){
		  ...
		  write_eth_param(full_udp_stack_ip_base_0, LOCAL_IP_ID, (ipAddress[0]<<24)|(ipAddress[1]<<16)|(ipAddress[2]<<8)|(ipAddress[3]));
		  call_params.local_ip = (ipAddress[0]<<24)|(ipAddress[1]<<16)|(ipAddress[2]<<8)|(ipAddress[3]);
		  call_params.stream_port = DEFAULT_PORT;
		  call_params.sync_port = DEFAULT_PORT;

		  write_switch_param(hyb_switch_ip_base_0, PORT1_ID, DEFAULT_PORT);
		  //		write_switch_param(hyb_switch_ip_base_0, PORT2_ID, sync_port);

		  write_eth_param(full_udp_stack_ip_base_0, DEST_PORT_ID, DEFAULT_PORT);
		  ...
		  write_eth_param(full_udp_stack_ip_base_0, SYNC_PORT_ID, DEFAULT_PORT);
	  }else{
		  error( "Failed to get gateway address, exiting, run dhclient manually: exiting!!\n");
		  sig_tpx("{\"tpx_msg\":\"bp__error\", \"code\":\"nolip\"}");
		  rc = (-4);
	  }
...
//			write_eth_param(full_udp_stack_ip_base_0, SYNC_PORT_ID, sync_port);
		...
		dest_port = dst_port;
		call_params.stream_port  = dst_port;
		call_params.sync_port  = dst_port;
...
void reset_call_params(){// only rempote data should be reset
	call_params.remote_pip = 0;
	call_params.remote_lip = 0;
	call_params.dest_ip = 0;
	call_params.sync_ip = 0;
//	call_params.local_ip = 0;
//	call_params.public_ip = 0;
//	call_params.sync_port = 0;
//	call_params.stream_port = 0;

	sig_tpx("{\"tpx_msg\":\"call__params__reset\"}");
}
...
int set_call_params(){
	...
	memset(lpip, 0, sizeof(lip));
	memset(ip, 0, sizeof(ip));
    //Check if local of public

	if((check_ip_sanity(gateway_ip) == 1) && (check_mac_sanity(local_mac) == 1)){
		info( "Gateway IP is good: %s", gateway_ip);
		info( "\nGood Mac Address: %02x. %02x. %02x. %02x. %02x. %02x\n",local_mac[0],local_mac[1],local_mac[2],local_mac[3],local_mac[4],local_mac[5]);
	}else{
		info( "ARP descriptors bad re-read from cache");
		set_mac_ip_net();
	}
    ...
    if((call_params.public_ip == call_params.remote_pip) && (call_params.remote_lip != 0)){//We are in the same network
    	print_ip(call_params.remote_lip, ip);
    	print_ip(call_params.local_ip, lip);
    	print_ip(call_params.public_ip, lpip);
    	info( "This is a local LAN IP run ARPING....");
    	info( "\nbridge_set_dest_ip(char * ip):Dest IP: %s :: Local IP: %s:: Local Pub IP: %s :: GW:%s\n",ip, lip, lpip, gateway_ip);
    	write_eth_param(full_udp_stack_ip_base_0, DEST_IP_ID, call_params.remote_lip);
//    	    run_arping("eth1", ip, ARP1_ID);//also writes to ARP Table in the FPGA
    	if(check_ip_net(ip, gateway_ip) == 1){//local IP
			info( "This is a local LAN IP run ARPING....");
			if(run_arping("eth1", ip, ARP1_ID) < 0){//also writes to ARP Table in the FPGA
				sig_tpx("{\"tpx_msg\":\"call__params\",\"error\":\"noarp\"}");
			}
		}else{//public so it just needs the gateway
			//read router MAC from ARP cache
			info( "This is a public IP take gateway....");
    	...
    	sig_tpx("{\"tpx_msg\":\"call__params\",\"destip\":\"%s\"}",ip);
    }else if((call_params.remote_pip != 0)){
    	...
    	sig_tpx("{\"tpx_msg\":\"call__params\",\"error\":\"noip\"}");
    	return -1;
    }

    if(call_params.stream_port != 0){
    	info("Port param set to: %d",call_params.stream_port);
    	call_params.sync_port = call_params.stream_port;
		write_switch_param(hyb_switch_ip_base_0, PORT1_ID, call_params.stream_port);
		write_switch_param(hyb_switch_ip_base_0, PORT2_ID, call_params.sync_port);

		write_eth_param(full_udp_stack_ip_base_0, DEST_PORT_ID, call_params.stream_port);
		write_eth_param(full_udp_stack_ip_base_0, SYNC_PORT_ID, call_params.sync_port);
		sig_tpx("{\"tpx_msg\":\"call__params\",\"port\":%d}",call_params.stream_port);
    }else{
    	sig_tpx("{\"tpx_msg\":\"call__params\",\"error\":\"noport\"}");
    	info("Port param cant be set to: %d",call_params.stream_port);
    	return -1;
    }
...
int bridge_init () {
	...
	if(hw_nodes_init() < 0){
		...
		sig_tpx("{\"tpx_msg\":\"bp_error\", \"code\":\"hw_nodes_init\"}");
		sig_tpx("{\"tpx_msg\":\"bp__exit\"}");
			  exit(-1);
	}
	...
	if(ret < 0){
		sig_tpx("{\"tpx_msg\":\"bp_error\", \"code\":\"net_init\"}");
		error("Failed to initialize hardware nodes: exiting!!\n");
		sig_tpx("{\"tpx_msg\":\"bp__exit\"}");
			  exit(ret);
	}
