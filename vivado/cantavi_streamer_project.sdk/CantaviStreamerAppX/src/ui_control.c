...
//			printf("axi_to_net_reg_w_error count is::%d\n",axi_to_net_reg_w_error_count);
		  ...
		  if(buffer&0x100000){
			  printf("Playout enabled :: val::0x%02X\n",buffer);
		  }
...
//                      	      	printf("Switch core reset status was:0x%X\n>",buf);
    ...
    //------------------------------------------------------------------------------------------------------
	...
	else if(strcmp (c,"cver") == 0)
			{
				  ...
				  printf("The FPGA core version is: %0.1f\n",tmp/10.0);
				  oled_clear(zedboard_oled_params_0.base_address);
			  ...
			  ui_draw();
				...
				tmp = get_media_latency(time_sync_base_0);
				  ...
				  printf("The media packets latency is: %0.1f ms\n",tmp/SAMPLE_RATE);
				  tmp = get_media_round_latency(time_sync_base_0);
				  printf("The media packets round latency is: %0.1f ms\n",tmp/SAMPLE_RATE);
				  oled_clear(zedboard_oled_params_0.base_address);
			...
			ui_draw();
      			  ...
      			  printf("Enable Audio payload generation\n");
      			oled_clear(zedboard_oled_params_0.base_address);
      			...
      			ui_draw();
      			  ...
      			  printf("Disable Audio payload generation \n");
      			oled_clear(zedboard_oled_params_0.base_address);
		  ...
		  ui_draw();
      		...
      		oled_print_message(&menuBuf[0], 1, zedboard_oled_params_0.base_address);
      		  ...
      		  ui_draw();
                    ...
                    if (c > 0){
                    	printf("Warning:: Bad packet length:: The packet length formula is:: 4xN + 14 \n Choose N wisely!!!\n");
                    }
...
//      			  }
    	  ...
    	  scanf ("%d",&tmp);
    	  set_playout_delay(eth_to_audio_base_0,(unsigned int)(tmp*SAMPLE_RATE));
    	  oled_clear(zedboard_oled_params_0.base_address);
...
//		  printf("The current audio rxtcfout is:%0.3f ms\n",(tmp/SAMPLE_RATE));
    	  ...
    	  tmp = get_dpkt_bad_pkts(eth_to_audio_base_0);
    	  printf("The number of dpkt bad pkts is:%d\n",tmp);
//    	  tmp = get_media_latency(time_sync_base_0);
...
//			printf("The current audio rxtc occ is:%d\n",(tmp));
      ...
      else if(strcmp (c,"udptsrx") == 0){
    	  unsigned d = read_eth_param(full_udp_stack_ip_base_0, RX_TS_REG_ID);
		//printf("\nMAC Address:");
//      		      		   print_mac(tmp, ip);
		  oled_clear(zedboard_oled_params_0.base_address);
		sprintf(&menuBuf[0], "%s", menuitem[11]);
...
//      			  }
      ...
      //      		      		   print_mac(tmp, ip);
      	      ...
      	      else if(strcmp (c,"getswrx") == 0) {
      	    	int reg = 0;
      	    	reg = read_switch_reg(hyb_switch_ip_base_0, 35);
      	    	info( "The ps_rx_pkt_count is :: %d", reg);
				reg = read_switch_reg(hyb_switch_ip_base_0, 36);
				info( "The cs_rx_pkt_count is :: %d", reg);
      	    	      //      		      		   print_mac(tmp, ip);
				  oled_clear(zedboard_oled_params_0.base_address);
				sprintf(&menuBuf[0], "%s", "SwRX:");
				oled_print_message(&menuBuf[0], 1, zedboard_oled_params_0.base_address);
				sprintf(&menuBuf[0], "%X", reg);
				oled_print_message(&menuBuf[0], 2, zedboard_oled_params_0.base_address);
      	    ...
      	    else if(strcmp (c,"getswtx") == 0) {
				int reg = 0;
				reg = read_switch_reg(hyb_switch_ip_base_0, 37);
				info( "The ps_tx_pkt_count is :: %d", reg);
				reg = read_switch_reg(hyb_switch_ip_base_0, 38);
				info( "The cs_tx_pkt_count is :: %d", reg);
					  //      		      		   print_mac(tmp, ip);
				  oled_clear(zedboard_oled_params_0.base_address);
				sprintf(&menuBuf[0], "%s", "SwTX:");
				oled_print_message(&menuBuf[0], 1, zedboard_oled_params_0.base_address);
				sprintf(&menuBuf[0], "%X", reg);
				oled_print_message(&menuBuf[0], 2, zedboard_oled_params_0.base_address);
      	  ...
      	  else if(strcmp (c,"getswst") == 0) {
      	  				int reg = 0;
      	  				reg = read_switch_reg(hyb_switch_ip_base_0, 37);
      	  				info( "The bus_status is :: 0x%X", reg);
      	  				reg = read_switch_reg(hyb_switch_ip_base_0, 38);
      	  				info( "The rx_sw_state is :: 0x%X", reg);
      	  					  //      		      		   print_mac(tmp, ip);
      	  				  oled_clear(zedboard_oled_params_0.base_address);
      	  				sprintf(&menuBuf[0], "%s", "SwST:");
      	  				oled_print_message(&menuBuf[0], 1, zedboard_oled_params_0.base_address);
      	  				sprintf(&menuBuf[0], "%X", reg);
      	  				oled_print_message(&menuBuf[0], 2, zedboard_oled_params_0.base_address);
...
//				  tmp = get_pkts_dropped(packet_time_enforcer_base_0);
            	tmp = get_pkts_dropped(eth_packet_sequencer_base_0);
				  printf("The number of sq packets dropped is:%u\n",tmp);
				  oled_clear(zedboard_oled_params_0.base_address);
...
//		set_port_address((ipAddress[0]<<24)|(ipAddress[1]<<16)|(ipAddress[2]<<8)|(ipAddress[3]), dest_port);
//	    sockfd = open_dummy_socket();
...
//	run_arping("eth1", dest_ip, ARP1_ID);
	...
	if(check_ip_net(dest_ip, gateway_ip) == 1){//local IP
		info( "The dest ip is a local LAN IP run ARPING....");
		run_arping("eth1", dest_ip, ARP1_ID);//also writes to ARP Table in the FPGA
	}else{//public so it just needs the gateway
		//read router MAC from ARP cache
		info( "This is a public IP take gateway....");
		update_arp_table(dest_ip,gateway_ip);
	...
	set_stream_send_off(audio_to_eth_base_0);
	set_stream_plen(audio_to_eth_base_0,(2 + 12+(4*48)));
...
int set_port_address(int ip_addr, int port){
