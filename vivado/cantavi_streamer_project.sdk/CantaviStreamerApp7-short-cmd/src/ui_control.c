      ...
      else if(strcmp (c,"setplcpktsize") == 0)
      {
				...
				ui_draw();
     				...
     				ui_draw();
							...
							//write_eth_param(full_udp_stack_ip_base_0, PKT_LEN_ID, ((tmp*3) +8));//+8 done in fpga
							//write_audio_to_eth_param(audio_to_eth_base_0, PAYLEN_REG_ID, tmp*3);
							//set_stream_pkt_wait(eth_packet_sequencer_base_0,tmp*(0x10000));//correct convcersion ms
...
//  saddr.port = atoi(argv[2]);
  ...
  printf ("Set local mac=%s::local address=%s:: dest address=%s:: and udp_port=%d\n",local_mac, local_ip, dest_ip, dest_port);
