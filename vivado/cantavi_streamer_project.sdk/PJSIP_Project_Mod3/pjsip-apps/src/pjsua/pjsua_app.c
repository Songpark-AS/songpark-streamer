...
pj_bool_t    sync_status	= PJ_FALSE;
pj_bool_t    streaming_status	= PJ_FALSE;
/*****************************************************************************
...
static void on_call_state(pjsua_call_id call_id, pjsip_event *e)
{
    ...
    if (call_info.state == PJSIP_INV_STATE_DISCONNECTED) {
    	...
    	PJ_LOG(4,(THIS_FILE,"-----------------on_call_state(pjsua_call_id call_id, pjsip_event *e)------------------\n\n\n\n"));

    	stop_hw_streaming();
    	deinit_incall_timed_task_handler();
    	sync_status	= PJ_FALSE;
    	streaming_status	= PJ_FALSE;
    	PJ_LOG(4,(THIS_FILE,"------------------------------------------------------------------------------------------\n\n\n\n"));
	    ...
	    call_info.state == PJSIP_INV_STATE_CONFIRMED) 
	{
	    ...
	    PJ_LOG(4,(THIS_FILE,"*************************TIMED CALL DURATION=%d*************************************",app_config.duration));
	}
	...
	if (call_info.state == PJSIP_INV_STATE_EARLY) {
		...
		PJ_LOG(3,(THIS_FILE, "Call %d \n conf media port: %d \n state changed to %.*s \n  remote info: %.*s\n local info: %.*s\n remote contact : %.*s  \n local contact : %.*s ",
					  ...
					  (int)call_info.local_contact.slen,
					  call_info.local_contact.ptr
		));
		 /****
		     *
		     * Call confirmed try the sync here
		     *
		     */
		//ToDO:: check originator so that we sync correctly

		if(!call_info.rem_offerer){//Only do sync if this was an out going call
			PJ_LOG(4,(THIS_FILE,"***************************************OUT GOING CALL INITIATE SYNC*************************************\n\n\n\n"));
	//		stop_hw_streaming();
			pj_status_t status = init_hw_sync();
			PJ_LOG(4,(THIS_FILE,"******************************************************************************************\n\n\n\n"));
			if (status == PJ_SUCCESS) {
				PJ_LOG(4,(THIS_FILE,"*************************************SYNC SUCCESS**********************************\n\n\n\n"));
				start_hw_streaming();//the other end is ready start sending samples
				streaming_status	= PJ_TRUE;
				sync_status	= PJ_TRUE;
				init_incall_timed_task_handler();
				PJ_LOG(4,(THIS_FILE,"******************************************************************************************\n\n\n\n"));
			} else {
				pjsua_perror(THIS_FILE, "Error initializing hardware sync", status);
				PJ_LOG(4,(THIS_FILE,"**************************************SYNC FAILED****************************************\n\n\n\n"));
				deinit_hw_sync();
				sync_status	= PJ_FALSE;
				streaming_status	= PJ_FALSE;
				PJ_LOG(4,(THIS_FILE,"******************************************************************************************\n\n\n\n"));
			...
			PJ_LOG(4,(THIS_FILE,"*********************************INCOMING CALL WAIT FOR SYNC********************************************\n\n\n\n"));
			start_hw_streaming();//the other end is ready start sending samples
			sync_status	= (bridge_is_timesync_good() > 0) ? PJ_TRUE : PJ_FALSE;
			streaming_status	= PJ_TRUE;
			init_incall_timed_task_handler();
			PJ_LOG(4,(THIS_FILE,"******************************************************************************************\n\n\n\n"));
		}
			     ...
			     pjsip_rx_data *rdata)
{
	...
	pj_status_t status;
		status = pjsua_call_answer2(call_id, &opt, app_config.auto_answer, NULL,
				   NULL);
    }
