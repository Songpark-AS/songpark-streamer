...
static void keystroke_help()
{
    ...
    puts("+-----------------------------------------------------------------------------+");
    puts("| pr  Codec path reset         | pt  Get timing info     | ps Manually sync  |");
    puts("| py  Reset sync               | pd  Set playout delay    | pm Manually stream|");
    puts("+-----------------------------------------------------------------------------+");
#if PJSUA_HAS_VIDEO
...
//	pjsua_call_answer2(current_call, &call_opt, st_code, NULL, &msg_data_);
	...
	pj_status_t status;
		status = pjsua_call_answer2(current_call, &call_opt, st_code, NULL, &msg_data);

//		if (status == PJ_SUCCESS) {
//				start_hw_streaming();
//			}else{
//				pjsua_perror(THIS_FILE, "Error answering call, not starting hw audio tx", status);
//			}
    }
}
...
static void ui_adjust_volume()
{
    char buf[128];
    char text[128];
    app_config.global_level = bridge_get_vol();
    sprintf(buf, "Adjust global vol: [%d] ", app_config.global_level);
    if (simple_input(buf,text,sizeof(text))) {
	char *err;
	app_config.global_level = (float)strtod(text, &err);
//	pjsua_conf_adjust_rx_level(0, app_config.mic_level);
	bridge_adjust_vol(app_config.global_level);
    }
...
//-----------------------------------------------------------------------------
static void ui_set_playout_delay()
{
    char buf[128];
    char text[128];
    int playout_delay = bridge_get_playout_delay();
    sprintf(buf, "Adjust playout delay: %f", playout_delay);
    if (simple_input(buf,text,sizeof(text))) {
	char *err;
	app_config.playout_delay = (float)strtod(text, &err);
	bridge_set_playout_delay(app_config.playout_delay);
    }else{
    	return;
    }
...
static void ui_manual_get_stream_params()
{
    char buf[128];
    char text[128];
//    int dest_port;
//    int sync_port;
//    char dest_ip[32];
    sprintf(buf, "Enter the destination IP:");
    if (simple_input(buf,text,sizeof(text))) {
	char *err;
//	app_config.dest_ip = (int)strtoi(text, &err);
	memset(app_config.dest_ip,0,sizeof(app_config.dest_ip));
	memcpy(app_config.dest_ip,text,sizeof(app_config.dest_ip));
//	bridge_set_playout_delay(app_config.playout_delay);
	bridge_set_dest_ip(app_config.dest_ip);
    }else{
    	return;
    }

    sprintf(buf, "Enter the destination Port:");
        if (simple_input(buf,text,sizeof(text))) {
    	char *err;
    	app_config.dest_port = (int)atoi(text);
    	bridge_set_dest_port(app_config.dest_port);
//    	bridge_set_playout_delay(app_config.playout_delay);
        }else{
        	return;
        }

	sprintf(buf, "Enter the sync Port:");
			if (simple_input(buf,text,sizeof(text))) {
			char *err;
			app_config.sync_port = (int)atoi(text);
			bridge_set_sync_port(app_config.sync_port);
//			bridge_set_playout_delay(app_config.playout_delay);
			}else{
				return;
			}
...
static void ui_manual_get_command_params()
{
    char buf[128];
    char text[128];
//    int dest_port;
//    int sync_port;
//    char dest_ip[32];
    sprintf(buf, "Enter the command and arguments:");
    if (simple_input(buf,text,sizeof(text))) {
	char *err;
//	app_config.dest_ip = (int)strtoi(text, &err);
//	memset(app_config.dest_ip,0,sizeof(app_config.dest_ip));
...
extern pj_bool_t streaming_status;
void get_call_status(){
	pjsua_call_info ci;
		if (pjsua_call_get_info(current_call, &ci)==PJ_SUCCESS){
			PJ_LOG(4,(THIS_FILE,"Current call id=%d with:: %.*s status:[%.*s] time_sync_status:%s streaming_status:%s\n", current_call,
			   (int)ci.remote_info.slen, ci.remote_info.ptr,
			   (int)ci.state_text.slen, ci.state_text.ptr,
			(bridge_is_timesync_good() > 0) ? "SYNC_GOOD" : "NO_SYNC",
					streaming_status == PJ_TRUE ? "ON" : "OFF"	));



	    }else{
	    	PJ_LOG(2,(THIS_FILE,"Cant get call information, most likely no active call..."));
	    }
...
void legacy_main(void)
{
    ...
    write_log_point(".....................SIP INIT DONE....................\n");
    for (;;) {

	printf(">>> ");
	fflush(stdout);

	if (fgets(menuin, sizeof(menuin)-1, stdin) == NULL) {
	    /*
  ...
  (!defined(PJ_WIN32_WINCE) || PJ_WIN32_WINCE==0)
	...
	if(strstr(menuin, "log_tpx") != NULL){
//		else if(strcmp (c,"log_tpx") == 0) {
		unsigned char desc_str[512]={0};
		memset(desc_str, 0, sizeof(desc_str));
		fprintf( stderr, "%s", menuin);
		if(first == 0){
				snprintf(desc_str, sizeof(desc_str), "TPX FIRST:: %s",menuin);
				write_log_point(desc_str);
				write_log_tpx(desc_str);
				first = 1;
		}else{
			 snprintf(desc_str, sizeof(desc_str), "%s",menuin);
			 write_log_tpx(desc_str);
		}
...
//							}
		...
		switch (menuin[0]) {
			...
			case 'd':
				...
				break;
				//All new commands directly to the bridge
			case 'p':
					if (menuin[1] == 'r') {
					//Codec path reset
						bridge_reset_rx_chain();
					} else if (menuin[1] == 't') {
					//Codec gettcvars
						bridge_get_tc_vars();
					}else if (menuin[1] == 's') {
					...
					//Set playout Delay
						ui_set_playout_delay();
					}
					else if (menuin[1] == 'm') {
					 //Manual start streaming
						ui_manual_get_stream_params();
		//		    	deinit_hw_sync(); done in init sync
						init_hw_sync();
						start_hw_streaming();
						init_incall_timed_task_handler();
						 ...
						 //Manual start streaming
							ui_manual_get_command_params();
			//		    	deinit_hw_sync(); done in init sync
		//					init_hw_sync();
		//					start_hw_streaming();

						}
					else if(menuin[1] == 'p'){//get call status
						get_call_status();
					}
					else if (menuin[1] == 'z') {//after call
						bridge_reinit();
					}
					else if (menuin[1] == 'v') {//version
									bridge_version();
								}
					else {
					printf("ERROR::command :%s is not defined", menuin);
					}
					break;
			...
			default:
				if (menuin[0] != '\n' && menuin[0] != '\r') {
				printf("ERROR::Invalid input %s", menuin);
				}
				keystroke_help();
				break;
			}
