...
/**
 * Create UDP stream transport. This is where we configure the FPGA with the port and IP address
 * after sanity checks. We should open the socket to keep the stack happy it panics if we do not
 * register an open socket. We should also trigger the sensing of sync packets after opening the socket.
 */
						  ...
						  pjmedia_transport **p_tp)
{
    ...
    /* Create RTP socket */
    status = pj_sock_socket(af, pj_SOCK_DGRAM_MED(), 0, &si.rtp_sock);
    ...
    /* Create RTCP socket */
    status = pj_sock_socket(af, pj_SOCK_DGRAM_SIG(), 0, &si.rtcp_sock);
    if (status != PJ_SUCCESS)
	goto on_error;

    /* Bind RTCP socket */
    printf("The Address is:: %s and the port is %d", addr->ptr, port);
    ...
    //Pass this information to the FPGA
    PJ_LOG(4,(THIS_FILE, "Setting HW Address is:: %s and the port is %d", addr->ptr, port));
    bridge_set_hw_dest_net(addr->ptr, port);
...
/*** block this section media is filtered by the switch on the FPGA it ends in the FPGA
    do {
						       ...
						       pj_ssize_t))
{
	...
	pj_uint16_t port = pj_sockaddr_get_port(&udp->rem_rtp_addr);
//		bit_ip2str(&bound_addr.ipv4.sin_addr, str);
	bit_ip2str(&udp->rem_rtp_addr.ipv4.sin_addr, str);
	PJ_LOG(4,(THIS_FILE, "Setting HW Address: bit::0x%X08 ::  str:: %s and the port is %d", (int)udp->rem_rtp_addr.ipv4.sin_addr.s_addr, str, port));
	bridge_set_hw_dest_net(str, port);
	PJ_LOG(4,(THIS_FILE, "\n\n\n\n******************************************************************************************\n\n\n\n\n"));
...
/* Called by application to send RTP packet:: This should be blocked to ensure that media comes from the FPGA */
static pj_status_t transport_send_rtp( pjmedia_transport *tp,
				       const void *pkt,
				       pj_size_t size)
{
...
***/
    ...
    //We block this from happening FPGA takes care of business so we do not want any packets from this layer
    /**
     *
				     ...
				     struct transport_udp *udp)
{
    ...
    *sock = PJ_INVALID_SOCKET;   
    ...
    status = pj_sock_socket(af, pj_SOCK_DGRAM_MED(), 0, sock);
