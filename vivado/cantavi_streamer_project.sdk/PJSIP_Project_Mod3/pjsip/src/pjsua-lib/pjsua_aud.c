				    ...
				    unsigned bits_per_sample)
{
    ...
    /* Create default parameters for the device THANX:: Audio device error
     * here FPGA is in cacharge we need to tie this up*/
...
PJ_DEF(pj_status_t) pjsua_set_snd_dev(int capture_dev,
				      int playback_dev)
{
	//Commenting this whole function out gives error Signal 11 after SDP negotiation, we need to go deeper
    ...
    //We also use a NULL sound device here since the FPGA has control use config
