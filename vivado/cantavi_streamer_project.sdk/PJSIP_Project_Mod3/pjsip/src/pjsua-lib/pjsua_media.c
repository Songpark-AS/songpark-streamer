					...
					pjmedia_sock_info *skinfo)
{
	...
	/* Create RTP socket. */
	status = pj_sock_socket(af, pj_SOCK_DGRAM_MED(), 0, &sock[0]);
	if (status != PJ_SUCCESS) {
	...
	//Pass this information to the FPGA :: no this is local binding
//	PJ_LOG(4,(THIS_FILE, "\n\n\n\n******************************************************************************************\n\n\n\n"));
//	char str[INET_ADDRSTRLEN];
//	bit_ip2str(&bound_addr.ipv4.sin_addr, str);
//	PJ_LOG(4,(THIS_FILE, "Setting HW Address: bit 0x%X ::  str:: %s and the port is %d", (int)bound_addr.ipv4.sin_addr, str, acc->next_rtp_port));
//	bridge_set_hw_dest_net(str, acc->next_rtp_port);
//	PJ_LOG(4,(THIS_FILE, "\n\n\n\n******************************************************************************************\n\n\n\n\n"));
	...
	/* Create RTCP socket. */
	status = pj_sock_socket(af, pj_SOCK_DGRAM_MED(), 0, &sock[1]);
	if (status != PJ_SUCCESS) {
	    pjsua_perror(THIS_FILE, "socket() error", status);
	    pj_sock_close(sock[0]);
	    return status;
...
//	char str[INET_ADDRSTRLEN];
//	bit_ip2str(&bound_addr, str);
//	printf("Setting HW Address is:: %s and the port is %d", str, acc->next_rtp_port);
//	bridge_set_hw_dest_net(str, acc->next_rtp_port);
