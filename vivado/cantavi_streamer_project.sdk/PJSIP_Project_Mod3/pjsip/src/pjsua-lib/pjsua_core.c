...
static pj_bool_t logging_on_rx_msg(pjsip_rx_data *rdata)
{
    ...
    bridge_set_contact_ip(rdata->msg_info.msg_buf, pjsip_rx_data_get_info(rdata));
    bridge_update_media_port(rdata->msg_info.msg_buf, pjsip_rx_data_get_info(rdata));
...
static pj_status_t logging_on_tx_msg(pjsip_tx_data *tdata)
{
    ...
    bridge_update_media_port(tdata->buf.start, pjsip_tx_data_get_info(tdata));
				       ...
				       pj_sockaddr *p_pub_addr)
{
    ...
    /* Create socket */
    status = pj_sock_socket(af, pj_SOCK_DGRAM_SIG(), 0, &sock);
    if (status != PJ_SUCCESS) {
