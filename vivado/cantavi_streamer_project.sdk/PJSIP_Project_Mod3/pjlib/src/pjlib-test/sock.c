...
static int simple_sock_test(void)
{
    ...
    types[1] = pj_SOCK_DGRAM_MED();
    types[2] = pj_SOCK_DGRAM_SIG();
...
static int udp_test(void)
{
    ...
    rc = pj_sock_socket(pj_AF_INET(), pj_SOCK_DGRAM_SIG(), 0, &ss);
    if (rc != 0) {
	app_perror("...error: unable to create socket", rc);
	return -100;
    }

    rc = pj_sock_socket(pj_AF_INET(), pj_SOCK_DGRAM_SIG(), 0, &cs);
    if (rc != 0)
	return -110;
    ...
    rc = send_recv_test(pj_SOCK_DGRAM_SIG(), ss, cs, &dstaddr, NULL,
                        sizeof(dstaddr));
    if (rc != 0)
	goto on_error;

    /* Test send/recv, with sendto and recvfrom */
    rc = send_recv_test(pj_SOCK_DGRAM_SIG(), ss, cs, &dstaddr,
    ...
    rc = send_recv_test(pj_SOCK_DGRAM_SIG(), ss, cs, NULL, NULL, 0);
    if (rc != 0)
	goto on_error;

    /* Test send/recv with send() and recvfrom */
    rc = send_recv_test(pj_SOCK_DGRAM_SIG(), ss, cs, NULL, &srcaddr,
                        sizeof(srcaddr));
    if (rc != 0)
	goto on_error;
#endif
