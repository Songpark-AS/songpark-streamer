...
static int compliance_test(pj_bool_t allow_concur)
{
    ...
    rc = pj_sock_socket(pj_AF_INET(), pj_SOCK_DGRAM_SIG(), 0, &ssock);
    if (rc==PJ_SUCCESS)
        rc = pj_sock_socket(pj_AF_INET(), pj_SOCK_DGRAM_SIG(), 0, &csock);
    else
...
static int unregister_test(pj_bool_t allow_concur)
{
    ...
    status = app_socket(pj_AF_INET(), pj_SOCK_DGRAM_SIG(), 0, SPORT, &ssock);
    if (status != PJ_SUCCESS) {
    ...
    status = app_socket(pj_AF_INET(), pj_SOCK_DGRAM_SIG(), 0, RPORT, &rsock);
    if (status != PJ_SUCCESS) {
	app_perror("Error initializing socket", status);
	return -130;
    }
...
static int many_handles_test(pj_bool_t allow_concur)
{
	...
	rc = pj_sock_socket(pj_AF_INET(), pj_SOCK_DGRAM_SIG(), 0, &sock[count]);
		      ...
		      int inactive_sock_count)
{
	...
	rc = pj_sock_socket(pj_AF_INET(), pj_SOCK_DGRAM_SIG(), 0, &inactive_sock[i]);
