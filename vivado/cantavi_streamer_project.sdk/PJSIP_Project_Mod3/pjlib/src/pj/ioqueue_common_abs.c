				        ...
				        pj_ioqueue_key_t *h)
{
	...
	/* Socket is writable. */
         ...
         */
        if (h->fd_type == pj_SOCK_DGRAM_MED() || h->fd_type == pj_SOCK_DGRAM_SIG()) {
		...
		if (send_rc==PJ_STATUS_FROM_OS(EPIPE) && !IS_CLOSING(h) &&
		    (h->fd_type==pj_SOCK_DGRAM_MED() || h->fd_type==pj_SOCK_DGRAM_SIG()))
		{
...
retry_on_restart:
    ...
    if (pj_list_empty(&key->write_list)) {
		...
		if (status == PJ_STATUS_FROM_OS(EPIPE) && !IS_CLOSING(key) &&
		    (key->fd_type == pj_SOCK_DGRAM_MED() || key->fd_type == pj_SOCK_DGRAM_SIG()))
		{
		    if (!restart_retry) {
