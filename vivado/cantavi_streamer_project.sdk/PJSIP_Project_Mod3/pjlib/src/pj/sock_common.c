                                      ...
                                      pj_sockaddr *p_dst_addr)
{
    ...
    status = pj_sock_socket(af, pj_SOCK_DGRAM_MED(), 0, &fd);
    if (status != PJ_SUCCESS) {
	return status;
    }
...
PJ_DEF(int) pj_SOCK_DGRAM_MED(void)
{
    return PJ_SOCK_DGRAM_MED;
}

PJ_DEF(int) pj_SOCK_DGRAM_SIG(void)
{
    return PJ_SOCK_DGRAM_SIG;
}
