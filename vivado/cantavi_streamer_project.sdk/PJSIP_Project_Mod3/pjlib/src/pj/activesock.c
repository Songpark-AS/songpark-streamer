					  ...
					  pj_activesock_t **p_asock)
{
    ...
    PJ_ASSERT_RETURN(sock_type==pj_SOCK_STREAM() ||
		     (sock_type==pj_SOCK_DGRAM_SIG()||sock_type==pj_SOCK_DGRAM_MED()), PJ_EINVAL);
					      ...
					      pj_sockaddr *bound_addr)
{
    ...
    status = pj_sock_socket(addr->addr.sa_family, pj_SOCK_DGRAM_MED(), 0,
			    &sock_fd);
    if (status != PJ_SUCCESS) {
	return status;
    }
    ...
    status = pj_activesock_create(pool, sock_fd, pj_SOCK_DGRAM_MED(), opt,
