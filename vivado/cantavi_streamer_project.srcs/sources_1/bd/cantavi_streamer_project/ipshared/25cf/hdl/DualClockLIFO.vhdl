...
signal PktCountRd   : unsigned (ADDR_WIDTH-PKT_LEN_WIDTH-1 downto 0); 
signal PktCountWr   : unsigned (ADDR_WIDTH-PKT_LEN_WIDTH-1 downto 0);
...
constant PKT_COUNT_MAX : natural := 2**(PktCountWr'length)-1;
...
--    end process;
 ...
 process (Clear_in, WClk) begin
        if Clear_in = '1' then
            ...
            if (rising_edge(WClk)) then
                if(Replace_inprogress_neg_edge_rclk = '1') then
                    PktCountRd <= (others=>'0');                               
...
--    clearReadCache <= '1' when (WriteEn_in = '1' and full = '0') and (RdCachedCaptured = '1') else '0';
 ...
 WritePktCountUpdate:   process (Clear_in, WClk) begin
        if Clear_in = '1' then
            ...
            if (rising_edge(WClk)) then  
                if(Replace_inprogress_neg_edge_rclk = '1') then
                    PktCountWr <= to_unsigned(0, ADDR_WIDTH-PKT_LEN_WIDTH);  -- set pkt count to  1 because we have one this edge must come after the tlast for the overlapped pkt                      
                elsif(updateWrPktCount = '1' and PktCountWr < PKT_COUNT_MAX) then
